...
for(const f of files){
  ...
  let backfilled = false;   // verified via manifest-recorded derivation, not a direct blob match
  if(!src2){
    if(SHALLOW){
  ...
  listing.push({ file: f, id, ver, commit: src2.commit, retired, backfilled });
  (byId[id] = byId[id] || []).push(ver);
}
...
const mdRows = listing.map(l => {
  ...
  return `| \`${l.file}\` | \`${l.commit.slice(0, 12)}\` | ${meta[0]} | ${meta.slice(1).join("|")}${l.retired ? " *(test since retired from the manifest — inert until the id returns)*" : ""}${l.backfilled ? " *(skill tags backfilled after the fact — bytes are a derivation of this commit's build, not that build itself; see testdata/archive/skill-backfills.json)*" : ""} |`;
});
const archiveMd =
`# testdata/archive — every superseded test build, pinned

Generated by \`archive-testdata.js\` — do not hand-edit. Each file is
byte-identical to the committed \`testdata/<testId>.js\` it once was, UNLESS
its row is marked "skill tags backfilled" — those derive from the commit
shown (their pre-backfill content) plus a recorded, re-derivable patch list
in \`testdata/archive/skill-backfills.json\` (CLAUDE.md's skill-tag backfill);
\`archive-testdata.js --verify\` re-checks that derivation byte-for-byte on
every run, same as it byte-checks every other row against real git history.
For an unmarked row the commit column is the newest commit at which that
exact content was current; for a marked row it is the commit the backfill
patched FROM. Review Mode and resume load these so an attempt always opens
on the exact content it was sat on (ATTEMPTS-SPEC §9) — the skill field is
the one exception, since it is classification metadata about a question,
not the question itself, and pinning exists to protect what a student saw
and answered, not what the app knows about it later.
